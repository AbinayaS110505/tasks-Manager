...
const SplashScreen = ({
  ...
  return <div className="min-h-screen bg-background flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className="text-center z-10">
        ...
        {isLoading ? <div className="w-64 h-2 bg-muted rounded-full overflow-hidden mb-8 mx-auto">
            <div className="progress-bar h-full w-1/3 rounded-full"></div>
          </div> : <div className="mb-8 h-2"></div>}

        {/* Login and Sign Up Buttons */}
        <div className="space-y-4">
          <Button onClick={() => handleLogin('login')} disabled={isLoading} className="google-btn w-64 h-14 text-lg font-medium">
            {isLoading ? <div className="flex items-center space-x-3">
                <div className="w-5 h-5 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
                <span>Connecting...</span>
              </div> : <div className="flex flex-col items-center space-y-1">
          ...
          <Button onClick={() => handleLogin('signup')} disabled={isLoading} variant="outline" className="google-btn w-64 h-14 text-lg font-medium">
